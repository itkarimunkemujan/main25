...
const PillNav: React.FC<PillNavProps> = ({
  ...
  const toggleMobileMenu = () => {
    ...
    if (hamburger) {
      ...
      const line2 = lines[1]
      if (newState) {
        if (line1) gsap.to(line1, { rotation: 45, y: 3, duration: 0.3, ease })
        if (line2) gsap.to(line2, { rotation: -45, y: -3, duration: 0.3, ease })
      } else {
        if (line1) gsap.to(line1, { rotation: 0, y: 0, duration: 0.3, ease })
        if (line2) gsap.to(line2, { rotation: 0, y: 0, duration: 0.3, ease })
      }
    }
  ...
  return (
    <div className="absolute top-[1em] left-0 z-[1000] flex w-full justify-center md:left-auto md:w-auto">
      <nav
        className={`box-border flex w-max items-center justify-center px-4 md:px-0 ${className}`}
        aria-label="Primary"
        style={cssVars}
      >
        {isRouterLink(items?.[0]?.href) ? (
          ...
          >
            <img
              ...
              alt={logoAlt}
              ref={logoImgRef}
              className={`block h-full w-full object-cover ${theme === "dark" ? "invert" : ""}`}
            />
          </Link>
        ) : (
          <a
